...
from player import Player, PlayerFailedException
...
def save_visualizer_data(board, result, f):
    ''' save board in a format compatible with nsasherr's visualizer '''
    from player import P0BOARD

    print >> f, board.width, board.height
    for line in board.board:
        print >> f, ''.join(line).translate(PBOARD)
...
def run_round(cmd1, cmd2, board, name1="Contestant 1", name2="Contestant 2", verbose=False, interactive=False, ansi=False, **kwargs):
    delay = 1.0/FPS
    try:
    ...
    #save_visualizer_data(gameboard, result, file("round.log", "w"))
